...
nothrow
{
version (Windows) extern (C) int _strnicmp(const char*, const char*, size_t) pure;
...
nothrow:
    ...
    extern (D) static bool equals(const(char)[] name1, const(char)[] name2) pure
    {
        ...
        version (Windows)
        {
            return _strnicmp(name1.ptr, name2.ptr, name1.length) == 0;
        }
        else
        {
            return strncmp(name1.ptr, name2.ptr, name1.length) == 0;
        }
    }
