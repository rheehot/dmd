    ...
    *dst = MatcherNode(isExclude, depth);
    ...
    if (depth > 0)
    {
        ...
        for (; dst < lastNode; dst++)
        {
            for (;; modulePattern++)
            {
                if (*modulePattern == '.')
                {
                    ...
                    *dst = MatcherNode(Identifier.idPool(idStart, cast(size_t)(modulePattern - idStart)));
                    modulePattern++;
        ...
        for (;; modulePattern++)
        {
            ...
            {
                assert(modulePattern > idStart, "empty module pattern");
                *lastNode = MatcherNode(Identifier.idPool(idStart, cast(size_t)(modulePattern - idStart)));
                break;
            }
        }
    }
